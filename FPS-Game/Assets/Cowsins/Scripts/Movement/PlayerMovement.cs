...
public class PlayerMovement : MonoBehaviour {
    ...
    #region variables
        ...
        [Tooltip("How much control you own while you are not grounded. Being 0 = no control of it, 1 = Full control.")][Range(0,1),SerializeField] 
        ...
        private float wallRunMinimumHeight;
    ...
    private void Update()
    {
        // Prevents from gaining really high values
        if (rb.velocity.magnitude > maxSpeedAllowed) rb.velocity = Vector3.ClampMagnitude(rb.velocity, maxSpeedAllowed);
        Stamina();
        if (canWallRun) CheckWalls();
        if(canWallBounce)CheckOppositeWall();

        if (InputManager.jumping && wallOpposite && canWallBounce && PlayerStats.Controllable && InputManager.y > 0) WallBounce();
    }
    ...
    public void HandleSpeedLines()
    {
        ...
        if (rb.velocity.magnitude > walkSpeed) speedLines.Play(); // Play speedlines
    ...
    private bool CheckHeight() { return !Physics.Raycast(transform.position, Vector3.down,wallRunMinimumHeight, whatIsWallRunWall); }
