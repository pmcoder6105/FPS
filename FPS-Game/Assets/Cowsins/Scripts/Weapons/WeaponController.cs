...
public class WeaponController : MonoBehaviour
{
    ...
    public void MeleeAttack(float attackRange,float damage)
    {
        ...
        foreach (var c in col)
        {
            ...
            {
                if(c.CompareTag("Critical")) c.transform.parent.GetComponent<IDamageable>().TakeDamage(dmg);
                else c.GetComponent<IDamageable>().TakeDamage(dmg);
            }
    ...
    private void Hit(LayerMask layer,float damage, RaycastHit h, bool damageTarget)
    {
        ...
        if (!damageTarget) return;
            if(h.collider.gameObject.CompareTag("Critical")) h.collider.transform.parent.GetComponent<IDamageable>().TakeDamage(damage * weapon.criticalDamageMultiplier * GetDistanceDamageReduction(h.collider.transform));
            else if (h.collider.GetComponent<IDamageable>() != null) h.collider.GetComponent<IDamageable>().TakeDamage(damage * GetDistanceDamageReduction(h.collider.transform));
    }
